...
* **Núcleo de Dados e Autenticação (Firebase):**
  * **Firestore (Banco de Dados):** Banco NoSQL em tempo real para persistência durável. Permite consultar, atualizar e armazenar informações de forma extremamente rápida com estrutura flexível.
...
* **Camada de Orquestração (Servidor Node.js + Express):** Executado em contêineres no Cloud Run para papéis fundamentais:
  ...
  * **Rotas de API:** Gerencia endpoints `/api/*` para ações específicas de lógica de negócio em ambiente controlado.
  * **Servidor Estático:** Em produção, gerencia a entrega dos arquivos do front-end, unificando a infraestrutura.
...
* **Responsabilidade Única:** Cada arquivo de serviço gerencia exclusivamente as interações com o Firebase, consultas, filtros e tratamentos de dados referentes ao seu respectivo domínio.
